...
def test_extract_null_txn_id() -> None:
    """CSV with null txn_id should be handled appropriately."""
    null_csv: str = """txn_id,account_id,txn_date,ingestion_date,amount,currency,txn_type,status
    ,acc1,2024-01-01,2024-01-01,100,EUR,CARD,COMPLETED
    t2,acc2,2024-01-02,2024-01-02,200,EUR,CARD,COMPLETED
    """
    # Currently extract allows null txn_ids - this documents the behavior
    df: pd.DataFrame = extract_transactions(io.StringIO(null_csv))
    assert len(df) == 2


def test_extract_empty_dataframe() -> None:
    """Empty CSV (only header) should return an empty DataFrame."""
    empty_csv: str = """txn_id,account_id,txn_date,ingestion_date,amount,currency,txn_type,status
    """
    df: pd.DataFrame = extract_transactions(io.StringIO(empty_csv))
    assert len(df) == 0
    assert list(df.columns) == [
        "txn_id",
        "account_id",
